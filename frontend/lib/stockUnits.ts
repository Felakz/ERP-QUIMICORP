...
export function stockUnit(unit?: string): string {
  ...
  if (['KG', 'KILO', 'KILOS', 'GR', 'G'].includes(u)) return 'GR';
  if (['L', 'LT', 'LITRO', 'LITROS', 'ML'].includes(u)) return 'ML';
  if (['UN', 'UND', 'UNIDAD'].includes(u)) return 'UN';
  return u;
}

export function unitFactor(unit?: string): number {
  return ['KG', 'KILO', 'KILOS', 'L', 'LT', 'LITRO', 'LITROS'].includes(String(unit).toUpperCase()) ? 1000 : 1;
}
