      ...
      {clientes.map((empresa) => {
        ...
        return (
          <div
            ...
            className={`rounded-2xl p-5 border space-y-4 transition-all card-hover-lift hover:border-cyan-500/60 hover:shadow-xl hover:shadow-cyan-500/10 cursor-pointer flex flex-col justify-between group ${cardBg}`}
          >
            <div className="space-y-3">
              {/* Encabezado de la Tarjeta con Badges Neón */}
              <div className={`flex items-start justify-between gap-3 border-b pb-3 ${isDark ? 'border-slate-800/60' : 'border-slate-200'}`}>
                <div>
                  <div className="flex items-center gap-2">
                    <span className="px-2 py-0.5 rounded text-[10px] font-mono font-bold bg-blue-500/10 text-cyan-400 border border-blue-500/30">
                      {empresa.ruc.startsWith('SIN-RUC') || empresa.ruc.startsWith('NO-RUC') || empresa.ruc === '-'
                        ? 'RUC -'
                        : `RUC ${empresa.ruc}`}
                    </span>
                    <span className="px-2 py-0.5 rounded-full text-[9px] font-bold uppercase bg-emerald-500/10 text-[#00F2C3] border border-emerald-500/30 flex items-center gap-1">
                      <ShieldCheck className="w-3 h-3 text-[#00F2C3]" />
                      <span>{empresa.estado || 'ACTIVO'}</span>
                    </span>
                  </div>
                  <h3 className={`text-sm font-black mt-1.5 line-clamp-1 group-hover:text-cyan-400 transition-colors ${textValue}`}>
                    {empresa.razonSocial}
                  </h3>
                </div>
                ...
                <button
                  ...
                  className="p-1.5 rounded-lg bg-blue-500/10 hover:bg-cyan-500/20 text-cyan-400 border border-blue-500/30 transition-all hover:border-cyan-400 active:scale-90"
              ...
              <div className={`pt-2 border-t ${isDark ? 'border-slate-800/60' : 'border-slate-200'}`}>
                <button
                  ...
                  className={`w-full flex items-center justify-between p-2 rounded-xl border text-xs transition-colors ${
                    isDark
                      ? 'bg-[#151D2A]/80 hover:bg-[#151D2A] border-[#1A2232] hover:border-cyan-500/40'
                        ...
                        >
                          ...
                          {cont.telefono && (
                            <a
                              ...
                              className="flex items-center gap-1 font-mono text-[11px] font-bold text-emerald-400 hover:underline bg-emerald-500/10 px-2.5 py-1 rounded-lg border border-emerald-500/30 hover:border-[#00F2C3] transition-all shadow-sm"
